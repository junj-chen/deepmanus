...
test("message_start is idempotent for the same message_id", () => {
  let s = reduceEvent([], { kind: "message_start", message_id: "m1", speaker: "a" });
  const before = s;
  s = reduceEvent(s, { kind: "message_start", message_id: "m1", speaker: "a" });
...
test("text_delta after a tool call opens a NEW text part (preserves order)", () => {
  let s = reduceEvent([], { kind: "message_start", message_id: "m1", speaker: "a" });
  s = reduceEvent(s, { kind: "text_delta", message_id: "m1", delta: "before" });
  s = reduceEvent(s, { kind: "tool_call_start", message_id: "m1", call_id: "t1", tool: "ls" });
  s = reduceEvent(s, { kind: "tool_call_end", message_id: "m1", call_id: "t1" });
  s = reduceEvent(s, { kind: "text_delta", message_id: "m1", delta: "after" });
...
test("thinking_delta accumulates separately from text", () => {
  let s = reduceEvent([], { kind: "message_start", message_id: "m1", speaker: "a" });
  s = reduceEvent(s, { kind: "thinking_delta", message_id: "m1", delta: "let me think" });
  s = reduceEvent(s, { kind: "thinking_delta", message_id: "m1", delta: "..." });
  s = reduceEvent(s, { kind: "text_delta", message_id: "m1", delta: "answer" });
  const m = s[s.length - 1];
...
test("tool_call lifecycle: start → args → result → end", () => {
  let s = reduceEvent([], { kind: "message_start", message_id: "m1", speaker: "a" });
  s = reduceEvent(s, { kind: "tool_call_start", message_id: "m1", call_id: "t1", tool: "read_file" });
  s = reduceEvent(s, { kind: "tool_call_args", call_id: "t1", args_json: '{"path":"a' });
  s = reduceEvent(s, { kind: "tool_call_args", call_id: "t1", args_json: '.py"}' });
  s = reduceEvent(s, { kind: "tool_call_result", call_id: "t1", result: "file contents" });
...
test("multiple concurrent tool calls match by their own call_id", () => {
  let s = reduceEvent([], { kind: "message_start", message_id: "m1", speaker: "a" });
  s = reduceEvent(s, { kind: "tool_call_start", message_id: "m1", call_id: "t1", tool: "ls" });
  s = reduceEvent(s, { kind: "tool_call_start", message_id: "m1", call_id: "t2", tool: "grep" });
  s = reduceEvent(s, { kind: "tool_call_args", call_id: "t2", args_json: "x" });
  s = reduceEvent(s, { kind: "tool_call_result", call_id: "t1", result: "r1" });
  const m = s[s.length - 1];
...
test("mailbox adds a distinct assistant bubble, deduped by id", () => {
  let s = reduceEvent([], { kind: "mailbox", session_id: "x", mailbox: { id: 5, kind: "result", content: "done", from_session_id: "abc12345" } });
  assert.equal(s.length, 1);
  assert.equal(s[0].id, "mb-5");
  // duplicate mailbox frame → ignored
  s = reduceEvent(s, { kind: "mailbox", session_id: "x", mailbox: { id: 5, kind: "result", content: "done", from_session_id: "abc12345" } });
  assert.equal(s.length, 1);
});
...
test("reduceEvent does not mutate the input array", () => {
  const base = reduceEvent([], { kind: "message_start", message_id: "m1", speaker: "a" });
  const frozen = [...base];
...
test("unknown event kinds are a no-op (return same ref)", () => {
  const s = [{ id: "m1", role: "assistant", speaker: "a", thinking: "", content: [], status: "streaming" }];
