 ...
 */
export async function sendMessage({ sessionId, content, signal, onEvent, onDone, onError }) {
  let res;
  try {
    res = await fetch(`${BACKEND}/sessions/${encodeURIComponent(sessionId)}/messages`, {
      method: "POST",
  ...
  if (!res.ok || !res.body) {
    if (typeof window !== "undefined") window.__ssErr = `status ${res.status}`;
 ...
 */
export function subscribe(sessionId, { scopeId, onEvent, onDone, onError } = {}) {
  const url = scopeId
